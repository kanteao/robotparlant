...
class RegexSemI(SemI):
    ...
    def init_regular_expressions(self):
        ...
        self.rBYE = "(\b|^|\ )(bye|goodbye|aurevoir|merci|that'?s?\ (is\ )*all)(\s|$|\ |\.)"
        self.GREAT = "(great|good|awesome)"
        self.HELPFUL = "(that((\')?s|\ (is|was))\ (very\ )?helpful)|(je\ suis\ satisfait)"
        self.THANK = "(thank(s|\ you)(\ (very|so)\ much)?)|(merci\ pour\ ton\ aide)"
        ...
        self.rREPEAT = "(\b|^|\ )(repeat\ that)|(say\ that\ again)|(encore)"
