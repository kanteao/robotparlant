...
def make_request_handler_class(dialServer):
    ...
    class RequestHandler(BaseHTTPServer.BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            logger.debug(u'Request: ' + str(request))
            logger.debug(u'POST full path: %s ' % self.path)
            if not 'Content-Length' in self.headers:
                data_string=self.path[self.path.find('?')+1:]
            else:
                data_string = self.rfile.read(int(self.headers['Content-Length']))

            logger.debug(u'Request Data:'+str(data_string).decode('utf8'))  # contains e.g:  {"session": "voip-5595158237"}
            ...
            try:
                ...
                logger.warning(u'Not a valid JSON object (or object lacking info) received. %s' % e)
            else:
            ...
            #------Work through cases: 1-fail, 2-new, 3-continue, 4-clean
                ...
                try:
                    ...
                    self.error_free = False
                    logger.warning(u'Tried to start a new call with a session id: {} already in use'.format(self.currentSession))
                    reply,prompt_str = dialServer.recognition_failed_response(msg='SESSION_ID_IN_USE', session_id=self.currentSession)
                else:
                    logger.info(u'A new call has started. Session: %s ' % self.currentSession)
                ...
                try:
                    ...
                    self.error_free = False
                    logger.warning(u'Tried to get an agent for the non-existent session id: {}'.format(self.currentSession))
                    reply,prompt_str = dialServer.recognition_failed_response(msg='NO_SUCH_SESSION', session_id=self.currentSession)
                else:
                    logger.info(u'Continuing session: %s with agent_id %s ' % (self.currentSession, agent_id))
                ...
                if self.error_free and 'result' in data and not dialServer.agent_factory.query_ENDING_DIALOG(agent_id):
                    if data['result'] =="ok":
                        ...
                        logger.info(u'ASR: None (ie voicebroker issued prompt only')
                        ...
                        logger.info(u'ASR: ' + str(asr_info))
                ...
                logger.info(u'Received request to Clean Session ID from the VoiceBroker...:' + self.currentSession)
                self.error_free = False                
                try:
            ...
            #FL> Add a separate request for Google ASR
            elif request=='google':
                import googleSR as gl
                result_from_google = gl.speechRecognition(data["data"], data["params"])
                reply = u'{"sentence":""}'
                try:
                    print (u''.join(result_from_google).encode('utf-8'))
                    reply = u'{"sentence":"'+u''.join(unicode(result_from_google))+u'"}'
                except:
                   pass
            ...
            if self.error_free and dialServer.agent_factory.query_ENDING_DIALOG(agent_id):
                logger.info(u'Agent %s is ending dialogue',agent_id)
                ...
                if dialServer.COLLECT_TASK and dialServer.agent_factory.agents[agent_id].task is None:
                    if dialServer.agent_factory.agents[agent_id].TASK_RETRIEVAL_ATTEMPTS == 0:
                        ...
                        logger.info(u'task_id %s' %task_id)
                    ...
                    if dialServer.agent_factory.agents[agent_id].TASK_RETRIEVAL_ATTEMPTS > 1:  # 1 : just ask for msg
                        if dialServer.agent_factory.agents[agent_id].task is None:
                            ...
                            logger.info(u'Successfully obtained a task')
                ...
                if dialServer.COLLECT_SUBJECTIVE_FEEDBACK and \
                    ...
                    if dialServer.agent_factory.agents[agent_id].subjective is None:
                        if dialServer.agent_factory.agents[agent_id].SUBJECTIVE_RETRIEVAL_ATTEMPS == 0:
                            ...
                            dialServer.agent_factory.agents[agent_id].subjective = dialServer.get_subjective_DTMF(agent_id,data,drange=["oui","Oui","non","Non"])
                            logger.info(u'Subjective feedback ' + str(dialServer.agent_factory.agents[agent_id].subjective))
                ...
                if TASK_LOGIC and FEEDBACK_LOGIC:
                    if 'local-audio' not in dialServer.agent_factory.agents[agent_id].session_id and dialServer.GENERATE_TOKEN:
                        ...
                        logger.info(u'Token for camdial feedback form: ' + str(token))
            ...
            logger.info(u'Sending prompt: '+prompt_str+' to tts.')
...
class DialogueServer(object):
    ...
    def run(self):
        ...
        logger.info(u'Server starting %s:%s (level=%s)' % (self.host, self.port, 'info'))
        try:
            while 1:
            ...
            logger.info(u'Server stopping %s:%s' % (self.host, self.port))
            server.server_close()
    ...
    def parse_additional_information(self, data):
        ...
        if "context" in data:
            start_domain = None
            logger.info(u'[DialPort]: context information received in JSON data')
            try:
                ...
                logger.warning(u'[DialPort]: Problem parsing dialport messages - JSON is missing the key: {}'.format(e))
            except Exception as e:
                logger.warning(u'[DialPort]: Problem parsing dialport messages {}'.format(e))
            ...
            if start_domain not in Ontology.global_ontology.ontologyManagers:  
                ...
                logger.warning(u'The received start domain: {} is not a valid domain'.format(start_domain))
                start_domain = None
            else:
                logger.info(u'External instruction to dialogue server to hand control to domain {}'.format(start_domain))
            info['start_domain'] = start_domain
        if "ood" in data:
            try:    
                if data["ood"]: # OOD detected
                    logger.debug(u'[DialPort]: ood detected')
    ...
    def ask_for_subjective(self,agent_id, currentPrompt):
        ...
        prompt_str = currentPrompt + u' ' + self.COLLECT_SUBJECTIVE_PROMPT  # now ask for 0/1
        session_id = self.agent_factory.agent2session(agent_id)
    ...
    def _get_DTMF(self,data):
        ...
        if "dtmf" not in result: 
            logger.warning(u'No dtmf entry from user/voiceserver') 
            return -1

        dtmf_input = result["dtmf"]
        logger.info(u'DialogueServer received DTMF : %s ' % dtmf_input)
        return dtmf_input
        ...
        try:
            ...
            if drange is not None:
                if int_dtmf in drange:
                    ...
                    logger.warning(u'Got a valid number - but not in data range {}.'.format(drange))
                    return None
            else:
                return int_dtmf
        except ValueError:
            logger.warning(u'DTMF Subjective error: non numeric')
        except:
            logger.warning(u'DTMF Subjective error')
    ...
    def get_task_id_DTMF(self, agent_id, data):
        ...
        try:
            ...
            logger.warning(u'You entered non numeric characters')  
        except KeyboardInterrupt:
            raise  # if the user is just pressing ctrl-c - let them quit!
        except:
            logger.warning(u'Something else went wrong - trying to get DTMF again') 
    ...
    def generate_token(self):
        ...
        token_str = u'  '.join(str(token))
        token_text = u'The 4 digit token for the feedback form is: ' + token_str + ' , '
        token_toRepeat = u' I repeat: ' + token_str + ' ,'
    ...
    def postToCamdial(self,token,dialogueID,task):
        """
        """
        param = self.TOKENSERVERURL + u'?token=' + str(token) + u'&dialogueID=' + dialogueID + u'&task=' + str(task)
        logger.info(u'MSG for Camdial: '+param)
        ...
        try:
            ...
            if "[Camdial] SAVED Token" not in r.text:   # this depends on the receive-token.py file camdial side
                logger.warning(u'Camdial communication error')
            else:
                logger.info(u'Success. Received following feedback from Camdial: '+r.text)
        except Exception as e:
            logger.warning(u'An error occured in communication with Camdial: '+str(e))
    ...
    def prompt(self,prompt, session_id, isbargein=None, isfinal=False, isdtmf=False, dtmfsize=1):
        ...
        if isbargein is None:
            isbargein = self.allowbargein  # default 
        reply={}
        reply['session']=u''+session_id
        reply['replyType']=u'prompt'
        reply['bargein']=u''+str(isbargein).lower()
        reply["final"]=u''+str(isfinal).lower()
        reply["dtmf"]=u''+str(isdtmf).lower()
        reply["dtmfsize"]=u''+str(dtmfsize)
        reply['text'] = u''+self._clean_text(prompt)
        return json.dumps(reply,encoding='utf8', ensure_ascii=False)
    ...
    def question(self,question, session_id, isbargein=None):
        ...
        reply={}
        reply['session']=u''+session_id
        reply['replyType']=u'question'
        reply['bargein']=u''+str(isbargein).lower()
        reply['text'] = u''+self._clean_text(question)
        return json.dumps(reply,encoding='utf8',ensure_ascii=False)
    ...
    def control(self,return_control, session_id, kept_alive):
        ...
        reply={}
        reply['session']=u''+session_id
        reply['replyType']=u'control'
        reply['returnControl']=u''+str(return_control)
        reply['keptAlive']=u''+str(kept_alive)
        return json.dumps(reply, encoding='utf8',ensure_ascii=False)
        ...
        for _list, like in tenBest:
            ...
            for i in _list:
                ...
                if word != "!NULL" and word != '<s>' and word != '</s>':
                    strutt+=' '+ word
            logger.debug(u'CNET: strut: ' + str(strutt))
            tenBestSents.append((strutt,like))

        logger.debug(u'CNET: top 10 sentences: ' + str(tenBestSents))
        return tenBestSents
    ...
    def _clean_text(self,RAW_TEXT):
        ...
        RAW_TEXT = RAW_TEXT.replace(u' !', u'!')
        RAW_TEXT = RAW_TEXT.replace(u' ?', u'?')
        return u''.join(c for c in RAW_TEXT if c not in JUNK_CHARS)
...
if __name__=="__main__":
    ...
    logger.info(u'Random Seed is {}'.format(seed))
