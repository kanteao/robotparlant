...
import StringIO
import fixed_tokenize as tokenize
...
class Scanner(object):
    ...
    def __init__(self, string):
        src = StringIO.StringIO(string).readline
    ...
    def check_tokens(self, tokens, message):
        ok = False
        for token in tokens:
            if type(token) == int:
                if self.cur[0] == token:
                ...
                if self.cur[1] == token:
                    ok = True
        if not ok:
            raise SyntaxError(message + '; tokens: %s' % str(self.cur))
