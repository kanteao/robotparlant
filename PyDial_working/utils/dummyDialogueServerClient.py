...
def run_fake_clients(NUM_CLIENTS=1, DIALOGS_PER_CLIENT=1, pause_time=0, hostname='localhost', port=8082):
    # PUT THE FAKE CLIENTS IN multiprocessing:
    output = mp.Queue()
    processes = []
    clients = {}
    for c in xrange(NUM_CLIENTS):
        clients[c] = fake_client(user_in=False,numDialogs=DIALOGS_PER_CLIENT, hostname=hostname, port=port)
